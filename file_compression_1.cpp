...
int main(){
    string text="this is a simple text which is going to be compressed by testing and correcting";
    ...
    int binaryTextpadding = 8-binaryText.length()%8;
    ...
    for(codes a: huffmanCode)
        binaryCode+=a.code;
    int binaryCodePadding=8-binaryCode.length()%8;
...
ofstream compressedfile("compressed file.bin",ios::binary);
uint8_t cc,cp,tp;
...
for(int i=0;i<charCount;i++){
    length=(uint8_t)huffmanCodes[i].code.length();
    compressedfile.write(reinterpret_cast<char*>(&huffmanCodes[i].ch), sizeof(huffmanCodes[i].ch));
    compressedfile.write(reinterpret_cast<char*>(&length),sizeof(length));
}
